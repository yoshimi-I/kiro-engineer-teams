...
7つのKiro CLIセッションを並列で起動し、issue→実装→レビュー→マージ→検証の全サイクルを自動化する。
...
[Agent 7] /improve ─────────── コード分析→改善issue自動生成

※ E2E-Hunt は watch-main に統合
...
Agent 7: /improve（10分間隔で改善issue自動生成）
```
...
| 7 | `/improve` | コード分析→改善issue自動生成 | 10分 |
